		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '⚠️ Este comando só pode ser usado por adms! 😝',
			...
			switch(command) {
				...
				case 'ep':
					if (args.length < 1) {
						...
						return reply('O texto também Mané')
					}
					reply(mess.wait)
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie foto/video com legendas ${prefix}sticker ou marque imagem/video que já foi enviado`)
					}
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Onde está o código do idioma?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto Mané', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
				...
				case 'meme':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'AHHAHAHAHAH VERRY FUNNY😂👋🏾'})
					break
				case 'memeindo':
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Lolis foram feitas para se dar carinho, não enfiar o pintinho'})
					})
					break
				case 'nsfwloli':
				...
				case 'limpar':
					...
					reply('Todos os Chats estão limpos :)')
					break
				case 'ts':
					if (!isOwner) return reply('Comando so funfa, se você for o dono do bot 😡')
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar o lek, deve ter privado, q fdp')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKKKK LEK TOMOU BAN, TROXA D+ : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'canal':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSGKPERXaVhs0FIiC_ixFSCho8poT0pcSewCg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do criador do bot:*\n\n https://youtube.com/channel/UCuL_XRKROorxaZCeV_Huejw'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Boas-vindas ativado neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Boas-vindas desativado neste grupo ✔️')
					} else {
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('marque alvo que você deseja clonar')

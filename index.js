...
const { randomNimek, msgFilter, ngtts } = require('./lib')
const fs = require('fs')
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'LegoBotV1'; if (!author) author = 'D370N4 F4C3';	
			...
			if (isCmd && msgFilter.isFiltered(from) && !isGroup) { return console.log(color('FLOOD AS', 'red'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'de', color(pushname)) }
		
		// ANTI FLOOD GRUPOS
        if (isCmd && msgFilter.isFiltered(from) && isGroup) { return console.log(color('FLOOD AS', 'red'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'de', color(pushname), 'em', color(name || formattedTitle)) }
		
        // MENSAGEM PV
        if (!isCmd && !isGroup) { return console.log('> MENSAGEM AS', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), 'de', color(pushname)) }
		
		// MENSAGEM GP
        if (!isCmd && isGroup) { return console.log('> MENSAGEM AS', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), 'de', color(pushname), 'em', color(name || formattedTitle)) }
		
		// COMANDOS
        if (isCmd && !isGroup) { console.log(color(`> COMANDO "${command} [${args.length}]" AS`), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), 'de', color(pushname)) }
		
		// COMANDOS GP
        if (isCmd && isGroup) { console.log(color(`> COMANDO "${command} [${args.length}]" AS`), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), 'de', color(pushname), 'em', color(name || formattedTitle)) }
        ...
        if (isCmd && !isOwner) msgFilter.addFilter(from)
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
				case 'tts':
                    if (args.length == 1) return reply(from, 'defina uma frase')
                    const dataText = body.slice(8)
                    var dataBhs = body.slice(5, 7)
			        if (dataText.length == '' || dataText.length > 500) return reply(from, 'Esqueceu do idioma/limite de 500 caracteres.', id)
			        const sppts = await ngtts(dataBhs, dataText)
			        console.log(sppts)
			        if (sppts == 'Error') return reply(from, `Hmm, '${dataBhs}' não é um idioma compativel, para idiomas compativeis digite ${prefix}idiomas.`, id)
			        await sleep(3000)
			        await sendPtt(from, `./lib/tts/res${sppts}.mp3`, id)
                    break
		        case 'onlyadms':
			        if (!isGroup) return reply(from, mess.only.group, id)
                    if (!isGroupAdmins) return reply(from, mess.only.admin, id)
                    if (!isBotGroupAdmins) return reply(from, mess.only.Badmin, id)
		         	if (args.length !== 1) return reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`, id)
                    if (args[0] == 'on') {
			    	await setGroupToAdminsOnly(groupId, true).then(() => sendText(from, 'Aqui está a prova de poder dos ademiros!\nO silenciador :O'))
		        	} else if (args[0] == 'off') {
			     	await setGroupToAdminsOnly(groupId, false).then(() => sendText(from, 'E os membros comuns podem voltar a badernar! e.e'))
		    	    } else {
			    	await reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`, id)
		         	}
			        break
                case 'mute':
                case 'silence':
			if (isGroup && isGroupAdmins || isGroup && isOwner) {
				...
				if (args[0] == 'on') {
					...
					await client.reply(from, 'Esse grupo não poderá mais usar os comandos.', id)
				} else if (args[0] == 'off') {
					slce.splice(groupId, 1)
					fs.writeFileSync('./lib/silence.json', JSON.stringify(slce))
					await client.reply(from, 'Esse grupo poderá usar os comandos novamente.', id)
				}
            } else {
                await client.reply(from, mess.only.admin, id)
            }
            break
                case 'nh':
			if (args.length == 1) {
				...
				if (cek == true)  {
					try {
						...
						await client.reply(from, '[❗] Ops! Deu erro no envio!', id)
					}
				} else {
					await client.reply(from, '[❗] Aqui diz que não achou resultados...')
				}
			} else {
				await client.reply(from, 'Insira um código do NHentai no comando para obter resultados.')
			}
			break
    	        case 'ihentai':
			    	...
			    	await client.sendFileFromUrl(from, hentai1.data.url, ``, 'Espero que curta o hentai e.e', id)
		         	}
                    break
                case 'yuri':
                ...
                case 'trap':
			     	...
			     	await client.sendFileFromUrl(from, trap, ``, '', id)
		        	}
                    break
                case 'google':
				    ...
				    await client.reply(from, vars, id)
                    }).catch(() => { client.reply(from, 'Erro ao pesquisar na google.', id) })
                    break
                case 'gadometro':
		        case 'gado':
			        ...
			        if (args.length == 1) {
				        await sendTextWithMentions(from, arqs[1] + ' é ' + lvpc + '% ' + gado + 'KKKKJ.')
			        } else {

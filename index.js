			...
			switch(command) {
                ...
                case 'lgbt':
                       const lvpc = Math.floor(Math.random() * 100) + 1
			 	    	...
			 	    	client.sendMessage(`Com base em meus estudos de Taxonomia e Quiropraxia, com os dados do meu cu, eu declaro que o @${id.split('@')[0]} é ${lvpc}% gay`, [jid], true)
        	 	     } else {
				 	 	reply(`Com base em meus estudos de Taxonomia e Quiropraxia, com os dados do meu cu, eu declaro que você é ${lvpc}% gay`)
       	 	 	     }
		 	 	 	  break
				case 'meme':
                ...
                case 'unblock':
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					client.unblockUser (`${body.slice(8)}@c.us`, "add")
					client.sendMessage(from, `Pedidos recebidos, ${body.slice(8)}@c.us foi desbloqueado`, text)
					break
				case 'porno':
                ...
                case 'leave':
					...
					client.sendMessage(from, 'Sayonara👋', text)
                ...
                case 'ping':    
                        ...
                        client.updatePresence(from, Presence.composing) 
				        uptime = process.uptime()
                        client.sendMessage(from, `Minha velocidade é de: *${latensi.toFixed(4)} _Second_*\`, text, { quoted: mek})
                        break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(mess.wait)

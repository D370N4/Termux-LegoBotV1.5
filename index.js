...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			if (args.length !== 1) return reply(`Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`)
            if (args[0] == 'on') {
				await client.setGroupToAdminsOnly(groupId, true).then(() => client.sendMessage(from, 'Aqui está a prova de poder dos ademiros!\nO silenciador :O'))
			} else if (args[0] == 'off') {
				await client.setGroupToAdminsOnly(groupId, false).then(() => client.sendMessage(from, 'E os membros comuns podem voltar a badernar! e.e'))
			} else {
				reply(`Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`)
			}
			break
                case 'figu':
				case 'figurinha':
		        ...
		        case 'gado':
			        ...
			        if (args.length == 1) {
			        	...
			        	reply(`Você é ` + lvpc + '% ' + gado + ' KKKKJ.')
		          	}
			        break
		        case 'math':
                    if (args.length == 0) return reply('Você não especificou uma conta matematica.')
                    const mtk = body.slice(6)
                    if (typeof math.evaluate(mtk) !== "number") {
			        	reply(`Você definiu mesmo uma conta? Isso não parece uma.`)
		         	} else {
		      		reply(`_A equação:_\n\n*${mtk}*\n\n_tem resultado de:_\n\n*${math.evaluate(mtk)}*`)
		        	}
		         	break
                case 'ship':
			        if (args.length !== 2) return reply('Faltou marcar o casal de pombinhos!')

	...
	client.on('chat-update', async (mek) => {
		try {
            ...
            if (args[0] == 'on') {
				...
				await reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`)
			}
			break
                case 'figu':
				case 'figurinha':
		        ...
		        case 'gado':
			        ...
			        if (args.length == 1) {
			        	...
			        	await reply(from, `Você é ` + lvpc + '% ' + gado + ' KKKKJ.')
		          	}
			        break
		        case 'math':
                    ...
                    if (typeof math.evaluate(mtk) !== "number") {
			        	await reply(from, `Você definiu mesmo uma conta? Isso não parece uma.`)
		         	} else {
		      		await reply(from, `_A equação:_\n\n*${mtk}*\n\n_tem resultado de:_\n\n*${math.evaluate(mtk)}*`)
		        	}
		         	break
                case 'ship':
			        if (args.length !== 2) return reply(from, 'Faltou marcar o casal de pombinhos!')
		        	await client.sendTextWithMentions(from, '❤️ ' + arqs[1] + ' tem um chance de ' + lvpc + '% de namorar ' + arqs[2] + '. 👩‍❤️‍👨')
		         	break	
                case 'gay':
                case 'lgbt':
		        	...
		        	var lvrq = 100 - lvpc
	          		if (args.length == 1 && isGroup) {
		         		...
		         		await reply(from, `Você é ${lvpc}% ${guei} e ${lvrq}% ${twgui}.`)
                    }
		         	break
				case 'tiktok':

		...
		try {
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
				case 'tts':
                    if (args.length == 1) return kill.reply(from, 'defina uma frase')
                    const dataText = body.slice(8)
                    var dataBhs = body.slice(5, 7)
			        if (dataText.length == '' || dataText.length > 500) return kill.reply(from, 'Esqueceu do idioma/limite de 500 caracteres.', id)
			        ...
			        await sleep(3000)
			        await kill.sendPtt(from, `./lib/tts/res${sppts}.mp3`, id)
                    break
		        case 'onlyadms':
			        if (!isGroup) return kill.reply(from, mess.error.Gp, id)
				...
				case 'setprefix':
					...
					break
				case 'randomloli':
		         	if (!isGroup && !isNsfw) return kill.reply(from, mess.error.Ac, id)

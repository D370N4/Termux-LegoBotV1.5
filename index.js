...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			if (args.length !== 1) return client.reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`)
            if (args[0] == 'on') {
				...
				await client.reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`)
			}
			break
                case 'figu':
				case 'figurinha':
		        ...
		        case 'gado':
			        ...
			        if (args.length == 1) {
			        	...
			        	await client.reply(from, `Você é ` + lvpc + '% ' + gado + ' KKKKJ.')
		          	}
			        break
		        case 'math':
                    if (args.length == 0) return client.reply(from, 'Você não especificou uma conta matematica.')
                    const mtk = body.slice(6)
                    if (typeof math.evaluate(mtk) !== "number") {
			        	await client.reply(from, `Você definiu mesmo uma conta? Isso não parece uma.`)
		         	} else {
		      		await client.reply(from, `_A equação:_\n\n*${mtk}*\n\n_tem resultado de:_\n\n*${math.evaluate(mtk)}*`)
		        	}
		         	break
                case 'ship':
			        if (args.length !== 2) return client.reply(from, 'Faltou marcar o casal de pombinhos!')
                ...
                case 'lgbt':
		        	...
		        	var lvrq = 100 - lvpc
	          		if (args.length == 1 && isGroupMsg) {
		         		...
		         		await client.reply(from, `Você é ${lvpc}% ${guei} e ${lvrq}% ${twgui}.`)
                    }
		         	break
				case 'tiktok':

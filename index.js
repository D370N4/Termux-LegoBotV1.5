		...
		try {
			...
			switch(command) {
				...
				case 'ocr':
					...
					break
                case 'nsfw':
                    if (args.length !== 1) return reply(from, 'Defina enable ou disable', id)
		         	if (isGroup && isGroupAdmins || isGroup && isOwner) {
			    	if (args[0].toLowerCase() == 'enable') {
				    	nsfw_.push(groupId)
				    	fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw_))
				     	await reply(from, 'Comandos NSFW ativados neste grupo!', id)
			     	} else if (args[0].toLowerCase() == 'disable') {
				    	nsfw_.splice(groupId, 1)
				    	fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw_))
				    	await reply(from, 'Comandos NSFW desativamos para este grupo.', id)
			    	} else {
				    	await reply(from, 'Defina enable ou disable', id)
			    	}
		     	} else if (!isGroup) {
			    	await reply(from, mess.error.Ga, id)
		    	} else {
			    	await reply(from, mess.error.Gp, id)
			       }
                   break
                case 'f':
                case 'figu':
				...
				case 'tts':
                    ...
                    break
		        case 'onlyadms':
			        if (!isGroup) return reply(from, mess.error.Gp, id)
                    if (!isGroupAdmins) return reply(from, mess.error.Ga, id)
                    if (!isBotGroupAdmins) return reply(from, mess.error.Ba, id)
    	        ...
    	        case 'ihentai':
			        ...
			        if (double == 1) {
		     		...
		     		await sendFileFromUrl(from, hentai, ``, 'Ui ui, hentai essa hora?', id)
	        		} else if (double == 2) {
			    	const hentai1 = await axios.get(hentcc)
			    	await sendFileFromUrl(from, hentai1.data.url, ``, 'Espero que curta o hentai e.e', id)
		         	}
                    break
                case 'yuri':
                    const yuri1 = await randomNimek('yuri')
                    await sendFileFromUrl(from, yuri1, ``, ``, id)
                    break
                case 'neko':
                case 'randomneko':
                    if (!isNsfw) return reply(from, mess.error.Ac, id)
			    	...
			    	await sendFileFromUrl(from, nekons.data.url, ``, '', id)
		        	} else if (double == 2) {
		    		const nsfwneko = await randomNimek('nsfw')
	    			await sendFileFromUrl(from, nsfwneko, ``, '', id)
		        	}
                    break
                case 'trap':
		         	if (!isNsfw) return reply(from, mess.error.Ac, id)
		      		...
		      		await sendFileFromUrl(from, tapr.data.url, '', '', id)
		         	} else if (double == 2) {
			     	const trap = await randomNimek('trap')
			     	await sendFileFromUrl(from, trap, ``, '', id)
		        	}
                    break
                case 'google':
                    if (args.length == 0) return reply(from, `Digite algo para buscar.`, id)
				    ...
				    await reply(from, vars, id)
                    }).catch(() => { reply(from, 'Erro ao pesquisar na google.', id) })
                    break
                case 'gadometro':
		        case 'gado':
			        ...
			        if (args.length == 1) {
				        ...
				        await reply(from, `Você é ` + lvpc + '% ' + gado + ' KKKKJ.', id)
			        }
			        break
				case 'meme':
                ...
                case 'morte':
		        case 'death':
                    if (args.length == 0) return reply(from, 'Coloque um nome, apenas um, nada de sobrenome ou nomes inteiros, ainda mais por sua segurança!', id)
			        const predea = await axios.get(`https://api.agify.io/?name=${args[0]}`)
			        await reply(from, `Pessoas com este nome "${predea.data.name}" tendem a morrer aos ${predea.data.age} anos de idade.`, id)
			        break		
				case 'loli1':
					memein = await kagApi.memeindo()
				...
				case 'setprefix':
					...
					break
				case 'randomloli':
		         	if (!isGroup && !isNsfw) return reply(from, mess.error.Ac, id)
			        const loliz = await axios.get('https://nekos.life/api/v2/img/keta')
			        await sendFileFromUrl(from, loliz.data.url, '', '', id)
			        break
				case 'nsfwloli':

		...
		try {
			...
			switch(command) {
				...
				case 'ocr':
					...
					break
                case 'onlyadms':
			        if (!isGroup) return client.reply(from, mess.only.group)
                    if (!isGroupAdmins) return client.reply(from, mess.only.admin)
                    if (!isBotGroupAdmins) return client.reply(from, mess.only.Badmin)
			        if (args.length !== 1) return client.reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`client.
                    if (args[0] == 'on') {
				        ...
				        await client.reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`client.
			        }
			        break
                case 'figu':
				case 'figurinha':
		        ...
		        case 'gado':
			        ...
			        if (args.length == 1) {
			        	...
			        	await client.reply(from, `Você é ` + lvpc + '% ' + gado + ' KKKKJ.'client.
		          	}
			        break
		        case 'math':
                    if (args.length == 0) return client.reply(from, 'Você não especificou uma conta matematica.'client.
                    const mtk = body.slice(6)
                    if (typeof math.evaluate(mtk) !== "number") {
			        	await client.reply(from, `Você definiu mesmo uma conta? Isso não parece uma.`client.
		         	} else {
		      		await client.reply(from, `_A equação:_\n\n*${mtk}*\n\n_tem resultado de:_\n\n*${math.evaluate(mtk)}*`client.
		        	}
		         	break
                case 'ship':
			        if (args.length !== 2) return kill.reply(from, 'Faltou marcar o casal de pombinhos!'client.
                ...
                case 'lgbt':
		        	...
		        	var lvrq = 100 - lvpc
	          		if (args.length == 1 && isGroupMsg) {
		         		await client.sendTextWithMentions(from, `${arqs[1]} é ${lvpc}% ${guei} e ${lvrq}% ${twgui}.`)
                    } else {
		         		await client.reply(from, `Você é ${lvpc}% ${guei} e ${lvrq}% ${twgui}.`client.
                    }
		         	break
				case 'tiktok':

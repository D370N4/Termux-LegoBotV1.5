		...
		try {
			...
			switch(command) {
				...
				case 'ocr':
					...
					break
                case 'onlyadms':
			if (!isGroup) return reply(from, mess.only.admin)
            if (!isGroupAdmins) return reply(from, mess.only.admin)
            if (!isBotGroupAdmins) return reply(from, mess.only.admin)
			if (args.length !== 1) return reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`)
            if (args[0] == 'on') {
				await client.setGroupToAdminsOnly(groupId, true).then(() => client.sendText(from, 'Aqui está a prova de poder dos ademiros!\nO silenciador :O'))
			} else if (args[0] == 'off') {
				await client.setGroupToAdminsOnly(groupId, false).then(() => client.sendText(from, 'E os membros comuns podem voltar a badernar! e.e'))
			} else {
				reply(from, `Você esqueceu de colocar se quer ativado [On], ou desativado [Off].`)
			}
			break
                case 'figu':
				case 'figurinha':
				...
				case 'mia2':
					...
					client.sendMessage(from, buffer, res.video, video, {quoted: mek, caption: 'use o .sticker para ver o gif da mia️'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
		        ...
		        case 'gado':
			        ...
			        if (args.length == 1) {
			        	client.sendMessage(from, arqs[1] + ' é ' + lvpc + '% ' + gado + 'KKKKJ.')
			       } else {
			        	reply(from, `Você é ` + lvpc + '% ' + gado + ' KKKKJ.')
		          	}
			        break
		        case 'math':
                    if (args.length == 0) return reply(from, 'Você não especificou uma conta matematica.')
                    const mtk = body.slice(6)
                    if (typeof math.evaluate(mtk) !== "number") {
			        	reply(from, `Você definiu mesmo uma conta? Isso não parece uma.`)
		         	} else {
		      		reply(from, `_A equação:_\n\n*${mtk}*\n\n_tem resultado de:_\n\n*${math.evaluate(mtk)}*`)
		        	}
		         	break
                case 'ship':
			        if (args.length !== 2) return reply(from, 'Faltou marcar o casal de pombinhos!')
		        	client.sendMessage(from,  '❤️ ' + arqs[1] + ' tem um chance de ' + lvpc + '% de namorar ' + arqs[2] + '. 👩‍❤️‍👨')
		         	break	
                case 'gay':
                case 'lgbt':
		        	...
		        	var lvrq = 100 - lvpc
	          		if (args.length == 1 && isGroup) {
		         		client.sendMessage(from, `${arqs[1]} é ${lvpc}% ${guei} e ${lvrq}% ${twgui}.`)
                    } else {
		         		reply(`Você é ${lvpc}% ${guei} e ${lvrq}% ${twgui}.`)
                    }
		         	break
				case 'tiktok':
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque alguém que ja esteja no grupo')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {

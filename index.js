		...
		try {
			...
			switch(command) {
				...
				case 'sticker':
					...
					} else if ((isMedia && mek.message.videoMessage.seconds > 12 || isQuotedVideo && mek.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 12) && args.length == 0) {
					reply(`O limite de tempo para uma figurinha animada é de 10 segundos, o recomendado é apenas 7 segundos no formado Gif`)
					}
					break
				case 'tts':
				   client.updatePresence(from, Presence.recording) 
				   if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem? Caso não saiba, use o conando ${prefix}idioma', text, {quoted: mek})
